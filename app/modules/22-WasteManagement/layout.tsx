...
import { Menu, X, ChevronLeft, ChevronRight, ChevronDown, AlertTriangle, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

const menuGroups = [
  { id: "waste-management", title: "قسم النفايات والإدارة الصحة", icon: AlertTriangle, items: ["قريباً ...."] },
]

export default function WasteManagementLayout({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["waste-management"])
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">قسم النفايات والإدارة الصحة</h1>
                <p className="text-sm text-yellow-100">قريباً ...</p>
              </div>
            </div>
            <Link href="/admin" className="text-yellow-100 hover:text-white transition-colors flex items-center gap-2"><ArrowLeft className="w-4 h-4"/>العودة للوحة الرئيسية</Link>
          </div>
        </div>
      </header>
