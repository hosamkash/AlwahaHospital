...
import { Menu, X, ChevronLeft, ChevronRight, ChevronDown, Activity, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

const menuGroups = [
  { id: "mental-health", title: "نظام إدارة الصحة النفسية", icon: Activity, items: ["قريباً ...."] },
]

export default function MentalHealthLayout({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["mental-health"])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-rose-50">
      <header className="bg-gradient-to-r from-pink-700 to-rose-800 text-white shadow-lg">
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">نظام إدارة الصحة النفسية</h1>
                <p className="text-sm text-pink-100">قريباً ...</p>
              </div>
            </div>
            <Link href="/admin" className="text-pink-100 hover:text-white transition-colors flex items-center gap-2"><ArrowLeft className="w-4 h-4"/>العودة للوحة الرئيسية</Link>
          </div>
        </div>
      </header>
