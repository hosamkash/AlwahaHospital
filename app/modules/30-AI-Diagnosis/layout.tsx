...
import { Menu, X, ChevronLeft, ChevronRight, ChevronDown, Brain, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

const menuGroups = [
  { id: "ai-diagnosis", title: "إدارة الذكاء الاصطناعي للتشخيص المبكر", icon: Brain, items: ["قريباً ...."] },
]

export default function AIDiagnosisLayout({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["ai-diagnosis"])
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">إدارة الذكاء الاصطناعي للتشخيص المبكر</h1>
                <p className="text-sm text-purple-100">قريباً ...</p>
              </div>
            </div>
            <Link href="/admin" className="text-purple-100 hover:text-white transition-colors flex items-center gap-2"><ArrowLeft className="w-4 h-4"/>العودة للوحة الرئيسية</Link>
          </div>
        </div>
      </header>
