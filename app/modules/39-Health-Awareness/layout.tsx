...
import { Menu, X, ChevronLeft, ChevronRight, ChevronDown, Megaphone, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

const menuGroups = [
  { id: "health-awareness", title: "نظام إدارة التوعية الصحية للمجتمع", icon: Megaphone, items: ["قريباً ...."] },
]

export default function HealthAwarenessLayout({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["health-awareness"])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-lime-50 to-green-50">
      <header className="bg-gradient-to-r from-lime-700 to-green-700 text-white shadow-lg">
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">نظام إدارة التوعية الصحية للمجتمع</h1>
                <p className="text-sm text-lime-100">قريباً ...</p>
              </div>
            </div>
            <Link href="/admin" className="text-lime-100 hover:text-white transition-colors flex items-center gap-2"><ArrowLeft className="w-4 h-4"/>العودة للوحة الرئيسية</Link>
          </div>
        </div>
      </header>
