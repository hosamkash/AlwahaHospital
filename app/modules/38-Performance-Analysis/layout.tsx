...
import { Menu, X, ChevronLeft, ChevronRight, ChevronDown, BarChart, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

const menuGroups = [
  { id: "performance-analysis", title: "تحليل البيانات لتقييم الأداء الطبي", icon: BarChart, items: ["قريباً ...."] },
]

export default function PerformanceAnalysisLayout({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["performance-analysis"])
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">تحليل البيانات لتقييم الأداء الطبي</h1>
                <p className="text-sm text-indigo-100">قريباً ...</p>
              </div>
            </div>
            <Link href="/admin" className="text-indigo-100 hover:text-white transition-colors flex items-center gap-2"><ArrowLeft className="w-4 h-4"/>العودة للوحة الرئيسية</Link>
          </div>
        </div>
      </header>
