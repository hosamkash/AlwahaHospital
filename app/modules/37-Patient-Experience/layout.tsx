...
import { Menu, X, ChevronLeft, ChevronRight, ChevronDown, UserPlus, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

const menuGroups = [
  { id: "patient-experience", title: "نظام إدارة تجربة المريض الشخصية", icon: UserPlus, items: ["قريباً ...."] },
]

export default function PatientExperienceLayout({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["patient-experience"])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 to-pink-50">
      <header className="bg-gradient-to-r from-rose-700 to-pink-800 text-white shadow-lg">
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">نظام إدارة تجربة المريض الشخصية</h1>
                <p className="text-sm text-rose-100">قريباً ...</p>
              </div>
            </div>
            <Link href="/admin" className="text-rose-100 hover:text-white transition-colors flex items-center gap-2"><ArrowLeft className="w-4 h-4"/>العودة للوحة الرئيسية</Link>
          </div>
        </div>
      </header>
