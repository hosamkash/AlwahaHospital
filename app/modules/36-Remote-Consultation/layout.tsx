...
import { Menu, X, ChevronLeft, ChevronRight, ChevronDown, Video, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

const menuGroups = [
  { id: "remote-consultation", title: "خدمة الاستشارات الطبية عن بُعد بتقنية الواقع المعزز", icon: Video, items: ["قريباً ...."] },
]

export default function RemoteConsultationLayout({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["remote-consultation"])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 to-purple-50">
      <header className="bg-gradient-to-r from-violet-700 to-violet-800 text-white shadow-lg">
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">خدمة الاستشارات الطبية عن بُعد</h1>
                <p className="text-sm text-violet-100">قريباً ...</p>
              </div>
            </div>
            <Link href="/admin" className="text-violet-100 hover:text-white transition-colors flex items-center gap-2"><ArrowLeft className="w-4 h-4"/>العودة للوحة الرئيسية</Link>
          </div>
        </div>
      </header>
