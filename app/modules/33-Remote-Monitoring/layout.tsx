...
import { Menu, X, ChevronLeft, ChevronRight, ChevronDown, Wifi, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

const menuGroups = [
  { id: "remote-monitoring", title: "مراقبة الحالة الصحية عن بُعد باستخدام أجهزة ذكية", icon: Wifi, items: ["قريباً ...."] },
]

export default function RemoteMonitoringLayout({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["remote-monitoring"])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 to-sky-50">
      <header className="bg-gradient-to-r from-cyan-700 to-cyan-800 text-white shadow-lg">
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">مراقبة الحالة الصحية عن بُعد باستخدام أجهزة ذكية</h1>
                <p className="text-sm text-cyan-100">قريباً ...</p>
              </div>
            </div>
            <Link href="/admin" className="text-cyan-100 hover:text-white transition-colors flex items-center gap-2"><ArrowLeft className="w-4 h-4"/>العودة للوحة الرئيسية</Link>
          </div>
        </div>
      </header>
