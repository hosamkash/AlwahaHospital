...
import { Menu, X, ChevronLeft, ChevronRight, ChevronDown, MapPin, ArrowLeft } from "lucide-react"
import Link from "next/link"
import { usePathname } from "next/navigation"

const menuGroups = [
  { id: "smart-tracking", title: "نظام التتبع الذكي للأدوية والمستلزمات", icon: MapPin, items: ["قريباً ...."] },
]

export default function SmartTrackingLayout({ children }: { children: React.ReactNode }) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["smart-tracking"])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-green-50">
      <header className="bg-gradient-to-r from-emerald-700 to-green-700 text-white shadow-lg">
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">نظام التتبع الذكي للأدوية والمستلزمات</h1>
                <p className="text-sm text-emerald-100">قريباً ...</p>
              </div>
            </div>
            <Link href="/admin" className="text-emerald-100 hover:text-white transition-colors flex items-center gap-2"><ArrowLeft className="w-4 h-4"/>العودة للوحة الرئيسية</Link>
          </div>
        </div>
      </header>
