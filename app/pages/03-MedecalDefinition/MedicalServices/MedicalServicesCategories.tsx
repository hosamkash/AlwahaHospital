...
import { MedicalServiceCategory } from "./list/medicalServicesController"

interface MedicalServicesCategoriesProps {
  categories: MedicalServiceCategory[]
  onSave: (category: MedicalServiceCategory) => void
  onDelete: (categoryId: number) => void
  onClose: () => void
}

export default function MedicalServicesCategories({ 
  categories, 
  onSave, 
  onDelete, 
  onClose 
}: MedicalServicesCategoriesProps) {
  const [showForm, setShowForm] = useState(false)
  const [selectedCategory, setSelectedCategory] = useState<MedicalServiceCategory | null>(null)
  const [isNewCategory, setIsNewCategory] = useState(false)
  const [formData, setFormData] = useState<MedicalServiceCategory>({
    id: 0,
    name: "",
    description: "",
    active: true
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-800">إدارة مجموعات الخدمات الطبية</h2>
