...
import AnalysisItem from "../form/analysisItem"
import { 
...
export default function Analysis({ onCreate, onEdit }: AnalysisProps) {
  const {
    analyses,
    showAnalysisDetails,
    selectedAnalysis,
  ...
  const filteredAnalyses = searchAnalyses(searchTerm)
      ...
      {showAnalysisDetails && selectedAnalysis ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-gray-800">عرض التحاليل الطبية</h2>
            <div className="flex items-center gap-2">
              <Button className="bg-primary" onClick={() => (onCreate ? onCreate() : handleNewAnalysis())}>
                  ...
                  <tbody>
                    {filteredAnalyses.map((analysis) => (
                      <tr key={analysis.id} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <Button 
                              size="sm" 
                              variant="outline"
                              onClick={() => (onEdit ? onEdit(analysis.id) : handleEditAnalysis(analysis))}
                            >
                        ...
                        <td className="py-3 px-4 text-gray-600 font-mono">{analysis.code}</td>
                        <td className="py-3 px-4 font-medium text-gray-800">{analysis.name}</td>
                        <td className="py-3 px-4">
                        ...
                        <td className="py-3 px-4 text-gray-600 max-w-xs truncate">{analysis.description}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
