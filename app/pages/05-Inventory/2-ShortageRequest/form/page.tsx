...
import { 
  ...
  defaultShortageRequests
} from "../ShortageRequestController"

function ShortageRequestFormContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const [request, setRequest] = useState<ShortageRequestDetails | null>(null)

  useEffect(() => {
    const idParam = searchParams.get("id")
    if (idParam) {
      const idNum = parseInt(idParam)
      const existing = defaultShortageRequests.find(r => r.id === idNum)
      if (existing) {
        // Map list item to full details shape
        setRequest({
          ...defaultShortageRequestDetails,
          id: existing.id,
          code: existing.code,
          ...
          employee: existing.responsibleEmployee,
    ...
    // Initialize with default data for new request
    setRequest({
      ...defaultShortageRequestDetails,
      id: Date.now(),
      code: `SR${Date.now()}`,
...
export default function ShortageRequestFormPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <ShortageRequestFormContent />
    </Suspense>
  )
}
