...
import { 
  PermissionInventoryAddDetails, 
  PermissionInventoryAddItem as PermissionItem,
  defaultPermissionInventoryAddDetails,
  usePermissionInventoryAddController
} from "../PermissionInventoryAddController"
import ProductSelectionDialog from "@/components/ProductSelectionDialog"
import ComboBox from "@/components/ComboBox"

function PermissionInventoryAddFormContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const permissionId = searchParams.get('id')

  const { permissionInventoryAdd, handleSavePermission } = usePermissionInventoryAddController()

  const [permission, setPermission] = useState<PermissionInventoryAddDetails | null>(null)
  ...
  useEffect(() => {
    if (permissionId) {
      const existingPermission = permissionInventoryAdd.find(perm => perm.id === parseInt(permissionId))
      if (existingPermission) {
        // For editing, load existing data and convert to PermissionInventoryAddDetails
        setPermission({
          ...defaultPermissionInventoryAddDetails,
          ...existingPermission,
          employee: existingPermission.responsibleEmployee, // Map responsibleEmployee to employee
          items: defaultPermissionInventoryAddDetails.items // Keep default items for now, or load actual items if available
        })
  ...
  }, [permissionId, permissionInventoryAdd])
  ...
  const handleSave = () => {
    if (permission) {
      ...
      router.push('/modules/05-Inventory/3-PermissionInventoryAdd/list')
    }
  }
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-800">
          {isNewPermission ? "إضافة إذن إضافة جديد" : "تعديل بيانات إذن الإضافة"}
        </h2>
...
export default function PermissionInventoryAddFormPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <PermissionInventoryAddFormContent />
    </Suspense>
  )
}
