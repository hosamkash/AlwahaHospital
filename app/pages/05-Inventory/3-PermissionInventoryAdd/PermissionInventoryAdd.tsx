...
import { 
  ...
  permissionInventoryAddUtils 
} from "./PermissionInventoryAddController"

type PermissionInventoryAddProps = {
  onCreate?: () => void
  onEdit?: (id: number) => void
}

export default function PermissionInventoryAdd({ onCreate, onEdit }: PermissionInventoryAddProps) {
  const {
  ...
  const filteredPermissions = searchPermissionInventoryAdd(searchTerm).filter(permission => {
    if (dateFilter === "today") {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-800">عرض إذون الإضافة</h2>
              ...
              <tbody>
                {filteredPermissions.map((permission) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-gray-600">{permissionInventoryAddUtils.formatDate(permission.date)}</td>
                    <td className="py-3 px-4 text-gray-600">{permission.time}</td>
                    <td className="py-3 px-4 font-medium text-gray-800">{permissionInventoryAddUtils.formatValue(permission.orderValue)}</td>
                    <td className={`py-3 px-4 font-medium ${permissionInventoryAddUtils.getStatusColor(permission.orderStatus)}`}>
