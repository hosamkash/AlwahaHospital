...
import EmployeesItem from "../form/employeesItem"
import { 
...
export default function Employees({ onCreate, onEdit }: EmployeesProps) {
  const {
    employees,
    showEmployeeDetails,
    selectedEmployee,
      ...
      {showEmployeeDetails && selectedEmployee ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-gray-800">عرض الموظفين</h2>
            <div className="flex items-center gap-2">
              <Button className="bg-primary" onClick={() => (onCreate ? onCreate() : handleNewEmployee())}>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      ...
                      <th className="text-right py-3 px-4 font-medium text-gray-700">الوظيفة</th>
                  ...
                  <tbody>
                    {filteredEmployees.map((employee) => (
                      <tr key={employee.id} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <Button 
                              size="sm" 
                              variant="outline"
                              onClick={() => (onEdit ? onEdit(employee.id) : handleEditEmployee(employee))}
                            >
                        ...
                        <td className="py-3 px-4 text-gray-600">{employee.code}</td>
                        <td className="py-3 px-4 text-gray-600">{employee.type}</td>
                        <td className="py-3 px-4 text-gray-600">{employee.department}</td>
                        <td className="py-3 px-4 text-gray-600">{employee.position}</td>
                        <td className="py-3 px-4 text-gray-600">{employeeUtils.formatSalary(employee.salary)}</td>
                        <td className="py-3 px-4">
