...
import GuarantorContextMenu from "../list/guarantorContextMenu"
import { 
  Guarantor, 
  branches, 
  priceCategories, 
  balanceTypes,
  guarantorUtils 
} from "../list/guarantorsController"

interface GuarantorsItemProps {
  selectedGuarantor: Guarantor | null
  isNewGuarantor: boolean
  onClose: () => void
  onSave?: (guarantor: Guarantor) => void
}

export default function GuarantorsItem({
  selectedGuarantor,
  isNewGuarantor,
  onClose,
  onSave,
}: GuarantorsItemProps) {
  const [guarantorData, setGuarantorData] = useState<Guarantor>(selectedGuarantor || {
    id: 0,
    branch: "مكه العمومي",
    code: 0,
    name: "",
  ...
  const handleInputChange = (field: keyof Guarantor, value: any) => {
  ...
  const handleContextMenuAction = (actionId: string) => {
    console.log(`تم اختيار: ${actionId} للمرافق:`, guarantorData.name)
    // هنا سيتم إضافة المنطق الخاص بكل إجراء
    handleCloseContextMenu()
  }

  if (!selectedGuarantor) return null
    ...
    <>
      ...
      <div className="bg-white border-b border-gray-200 p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-blue-800">
            {isNewGuarantor ? "إضافة مرافق جديد" : "تعديل بيانات المرافق"}
          </h2>
      ...
      <div className="bg-gray-50 p-6 rounded-b-lg">
        <div className="bg-white p-6 rounded-lg">
          <h3 className="text-lg font-bold text-gray-800 mb-6">بيانات المرافق</h3>
