...
import DoctorsItem from "../form/doctorsItem"
import { 
  useDoctorsController, 
  doctorUtils 
} from "./doctorsController"
...
export default function Doctors({ onCreate, onEdit }: DoctorsProps) {
  const {
    doctors,
    showDoctorDetails,
    selectedDoctor,
      ...
      {showDoctorDetails && selectedDoctor ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-gray-800">عرض الأطباء</h2>
            <div className="flex items-center gap-2">
              <Button className="bg-primary" onClick={() => (onCreate ? onCreate() : handleNewDoctor())}>
                  ...
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      ...
                      <th className="text-right py-3 px-4 font-medium text-gray-700">الكود</th>
                                             <th className="text-right py-3 px-4 font-medium text-gray-700">التخصص</th>
                       <th className="text-right py-3 px-4 font-medium text-gray-700">نشط</th>
                      <th className="text-right py-3 px-4 font-medium text-gray-700">رسوم الاستشارة</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredDoctors.map((doctor) => (
                      <tr key={doctor.id} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <Button 
                              size="sm" 
                              variant="outline"
                              onClick={() => (onEdit ? onEdit(doctor.id) : handleEditDoctor(doctor))}
                            >
                        ...
                        <td className="py-3 px-4 text-gray-600">{doctor.id}</td>
                        <td className="py-3 px-4 font-medium text-gray-800">{doctor.name}</td>
                        <td className="py-3 px-4 text-gray-600">{doctor.code}</td>
                        ...
                        <td className="py-3 px-4 text-gray-600">{doctorUtils.formatConsultationFee(doctor.consultationFee)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
