...
import StoresItem from "../form/storesItem"
import { 
  useStoresController, 
  storeUtils 
...
export default function Stores() {
  ...
  const filteredStores = searchStores(searchTerm)
      ...
      {showStoreDetails && selectedStore ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-gray-800">عرض المخازن</h2>
            <div className="flex items-center gap-2">
              <Button className="bg-primary" onClick={handleNewStore}>
                  ...
                  <tbody>
                    {filteredStores.map((store) => (
                      <tr key={store.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 text-gray-600">{store.code}</td>
                        <td className="py-3 px-4 font-medium text-gray-800">{store.name}</td>
                        <td className="py-3 px-4">
                        ...
                        <td className="py-3 px-4 text-gray-600 max-w-xs truncate">{store.description}</td>
                        <td className="py-3 px-4 text-gray-600">{getBranchName(store.branchId)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
