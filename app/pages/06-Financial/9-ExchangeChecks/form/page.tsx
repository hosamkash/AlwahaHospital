...
function ExchangeCheckFormContent() {
  const router = useRouter()
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const checkId = searchParams.get('id')

  const { exchangeChecks, handleSaveExchangeCheck } = useExchangeChecksController()

  const [check, setCheck] = useState<ExchangeCheckDetails | null>(null)
  ...
  // ComboBox options
  const branchOptions = exchangeChecksUtils.branchOptions
  const bankOptions = exchangeChecksUtils.bankOptions
  const entityTypeOptions = exchangeChecksUtils.entityTypeOptions

  useEffect(() => {
    if (checkId) {
      const existingCheck = exchangeChecks.find(chk => chk.id === parseInt(checkId))
      if (existingCheck) {
        // For editing, load existing data and convert to ExchangeCheckDetails
        setCheck({
          ...defaultExchangeCheckDetails,
          ...existingCheck,
          checkNumber: 0,
  ...
  const getListPath = () => {
    ...
    return basePath || "/modules/06-Financial/9-ExchangeChecks"
  }

  const handleSave = () => {
    if (check) {
      handleSaveExchangeCheck(check)
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-800">
          {isNewCheck ? "بيانات شيك الدفع - الشيك الصادر" : "تعديل بيانات شيك الدفع"}
        </h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="text-blue-600 font-bold text-lg">
              {exchangeChecksUtils.formatBalance(check.balance)}
            </div>
          </div>
          <div>
...
export default function ExchangeCheckFormPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <ExchangeCheckFormContent />
    </Suspense>
  )
}
